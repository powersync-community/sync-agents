...
1. **Workspace skill checked first** - If `{CONFIG_DIR}/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
{CONFIG_DIR}/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p {CONFIG_DIR}/workspaces/{ws}/skills/my-skill
```
...
1. Create `{CONFIG_DIR}/workspaces/{ws}/skills/commit/SKILL.md`
