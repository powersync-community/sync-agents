...
3. **Preset themes**: `{CONFIG_DIR}/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `{CONFIG_DIR}/theme.json` - Override specific colors (app-level)
...
```
{CONFIG_DIR}/workspaces/{id}/config.json
```
...
Create `{CONFIG_DIR}/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `{CONFIG_DIR}/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `{CONFIG_DIR}/themes/{name}.json`
...
1. Create `{CONFIG_DIR}/theme.json` for overrides or `{CONFIG_DIR}/themes/{name}.json` for a preset
...
- Check file is in correct location (`{CONFIG_DIR}/theme.json` for overrides, `{CONFIG_DIR}/themes/` for presets)
