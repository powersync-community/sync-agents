...
When you set `colorTheme` in `{CONFIG_DIR}/config.json`, it now properly takes priority over any cached theme in localStorage. This makes it possible for agents to modify the theme via documented config paths.
