  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See docs/skills.md in the config directory for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
...
/**
 * Zod schema for app-level theme override files ({CONFIG_DIR}/theme.json).
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in the tool-icons directory`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR.
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - {CONFIG_DIR}/tool-icons/tool-icons.json → tool icon mappings
 */
